  ...
  subscribe(context, callable) {
    if (!this.hasSubscribers()) {
      ...
      for (let i = 0, ii = dependencies.length; i < ii; i++) {
        let observer;
        if (/[a-z$_][a-z0-9$_?]*/i.test(dependencies[i])) {
          observer = this.observerLocator.getObserver(this.obj, dependencies[i]);
        } else {
          observer = new ExpressionObserver(this.obj, this.parser.parse(dependencies[i]), this.observerLocator);
        }
